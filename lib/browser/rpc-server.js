...
handleRemoteCommand('ELECTRON_BROWSER_REQUIRE', function (event, contextId, module) {
  return valueToMeta(event.sender, contextId, electron.app.remoteRequireHandler(event.sender, module))
})
...
handleRemoteCommand('ELECTRON_BROWSER_GLOBAL', function (event, contextId, name) {
  return valueToMeta(event.sender, contextId, electron.app.remoteGetGlobalHandler(event.sender, name))
})
